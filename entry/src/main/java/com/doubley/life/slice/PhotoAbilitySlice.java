...
import com.doubley.life.PhotoAbility;
...
public class PhotoAbilitySlice extends AbilitySlice {
    private static final String TAG = PhotoAbility.class.getSimpleName();
    ...
    public void onStart(Intent intent) {
        ...
        DirectionalLayout layout = (DirectionalLayout) LayoutScatter.getInstance(this)
                .parse(ResourceTable.Layout_layout_toast_photo, null, false);


        initComponents();
        initSurface();
        Image exitImage = (Image) findComponentById(ResourceTable.Id_exit);
        exitImage.setClickedListener(new Component.ClickedListener() {
            @Override
            public void onClick(Component component){
             if(saveFile.getPath()==null)
                 DataUtil.addSource(new Resource());
             terminateAbility();
            }
        });
    }
    ...
    private void saveImage(ImageReceiver receiver) {      //照片路径存储
        saveFile = new File(getExternalFilesDir(null), "IMG_" + System.currentTimeMillis() + ".jpg");
        ...
        try (FileOutputStream output = new FileOutputStream(saveFile)) {
            String source=saveFile.getPath();
            output.write(bytes);
            output.flush();
            String msg = "Take photo succeed";
            showTips(this, msg);
            DataUtil.addSource(new Resource(source));
        } catch (IOException e) {
    ...
    private class SurfaceCallBack implements SurfaceOps.Callback {
        ...
        public void surfaceCreated(SurfaceOps callbackSurfaceOps) {
            ...
            eventHandler.postTask(PhotoAbilitySlice.this::openCamera, 200);
        }
